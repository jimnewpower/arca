...
aws_account_number=$(aws sts get-caller-identity --query Account --output text)
host="host/originApplication/$aws_account_number/originApplication"

echo ""
echo "!!!!!!!!!!!!!!!!!!!!!!!"
echo "Copy the api_key above."
echo "!!!!!!!!!!!!!!!!!!!!!!!"
echo "2. Logon to Conjur as host/originApplication/$aws_account_number/originApplication"
echo "3. Set secret values in Conjur: connectionstring, username, password"
echo ""
echo "e.g. conjur set-secret -i originApplication/connectionstring -v connectionstring"
echo ""
